...
export default function AuthScreenProduction() {
  ...
  return <div className="auth-wrapper"><img src="/logo.png" alt="Aahat" className="auth-logo"/><h2 className="auth-title">{mode==='login'?'Welcome back':mode==='register'?'Create account':mode==='otp'?'Verify account':'Reset password'}</h2>
    {message&&<div className={`auth-${message.type}`}>{message.text}</div>}<form onSubmit={submit} className="auth-form">
    ...
    {mode==='login'&&<><button className="auth-link-btn" onClick={()=>setMode('forgot')}>Forgot password?</button><button className="auth-link-btn" onClick={()=>setMode('register')}>Create an account</button></>}
    {mode!=='login'&&<button className="auth-link-btn" onClick={()=>setMode('login')}><ArrowLeft size={14}/>Back to sign in</button>}
  </div>;
}
