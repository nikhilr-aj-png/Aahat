...
test('trusted server time gates the app and timestamps follow device hour format', async () => {
  const [migration, gate, main, formatter, bubble, conversations, chat] = await Promise.all([
  ...
  assert.match(formatter, /resolved\.hourCycle/);
  assert.match(main, /refreshDeviceTimeFormat\(\)/);
  assert.match(bubble, /formatDeviceTime\(msg\.created_at\)/);
  assert.match(conversations, /formatDeviceTime\(msg\.created_at\)/);
  assert.match(chat, /formatDeviceTime\(seen\)/);
});
