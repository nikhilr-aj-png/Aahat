...
test('voice and video calling attach remote audio and recover signaling gaps', async () => {
  ...
  assert.match(overlay, /audioMode === 'normal' \? 0\.45 : 1/);
  assert.match(overlay, /audioMode === 'normal' \? 'loud'/);
  assert.match(overlay, /audioMode === 'loud' \? 'muted'/);
  assert.match(overlay, /track\.addEventListener\('unmute', handleReady\)/);
  ...
  assert.match(secureCalling, /waitForIceGatheringComplete\(pc\)/);
